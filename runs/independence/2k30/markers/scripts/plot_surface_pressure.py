...
norms = numpy.array([numpy.linalg.norm([xi, yi]) for xi, yi in zip(x_n, y_n)])
x_n /= norms
y_n /= norms
...
filepath = simudir / 'data' / 'krishnan_et_al_2014_surface_pressure_2k30.txt'
with open(filepath, 'r') as infile:
    pk, xk, yk = numpy.loadtxt(infile, usecols=(0, 2, 3), unpack=True)
xk += abs(xk.min()) - abs(xb_m.min())  # x-displacement to align with us
...
ax.plot(xb_m, p_interp, color='C0', label='Present')
ax.plot(xk, pk, color='black', linestyle='--', label='Krishnan et al. (2014)')
ax.legend(frameon=False)
ax.axis((-0.6, 0.6, -1.5, 1.5))
fig.tight_layout()
